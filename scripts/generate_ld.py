...
def writeall(args):
    ...
    if os.path.basename(ldFile) == 'rom_gen.ld':
        armipsFile = 'armips/include/generated/symbols.s'
        valid_symbol = re.compile(r'^[A-Za-z_][A-Za-z0-9_]*$')
        with open(armipsFile, 'w') as armipsSymbols:
            armipsSymbols.write("// autogenerated by scripts/generate_ld.py\n")
            for key in sorted(table.keys()):
                if valid_symbol.match(key):
                    armipsSymbols.write(".definelabel " + key + ", " + hex(table[key]) + "\n")
